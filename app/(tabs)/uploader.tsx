...
export default function Uploader() {
    const [csuri, setCsuri] = useState('')
    const [isPicked, setIsPicked] = useState(false)
    const pagecon = usePage();
    if (!pagecon) return null
    const { setTitle } = pagecon
    const [files, setFiles] = useState<extendedpic[]>([])

    const handleUpload = async () => {
        return await handleFileUpload({ files, setCsuri, setFiles, setIsPicked })
    }
                ...
                {(!isPicked) ?
                    ...
                    <View className='flex w-full'>
                        ...
                        <View className='flex flex-row items-center gap-x-2'>
                            <TouchableOpacity activeOpacity={0.9} onPress={() => handleUpload()} className='bg-green-500/25 px-4 py-2 rounded' ><Text className='text-base text-green-500'><Feather name='upload' size={20} /> More</Text></TouchableOpacity>
                            <TouchableOpacity activeOpacity={0.9} onPress={() => { setFiles([]); setIsPicked(false) }} className='bg-red-500/25 px-4 py-2 rounded'><Text className='text-base text-red-500'>Clear</Text></TouchableOpacity>
                        </View>
                        <Text className='my-1 text-lg'>Recent Uploaded Pdf's</Text>
                        <View className='w-full flex flex-row flex-wrap gap-2'>
                            {
                                files.length > 0 && files.map((file, i) =>
                                    <View key={i} className='flex max-w-[130px] h-[130px] p-2 w-full'>
